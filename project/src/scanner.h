...
//constructor for Token 
   ...
   void init (string lexeme, tokenType terminal, Token* next);
	//Token (string lexeme, tokenType terminal, Token *next, char basic_constructor);
};
...
class TokenType {
	public:
		TokenType() {};
		TokenType(regex_t* lexeme_pattern, tokenType terminal, 
				bool (*err_checking_func)(int numMatchedChars,string lexeme), const char* err_string);
				
		TokenType(tokenType terminal, regex_t* lexeme_pattern, 
				bool (*err_checking_func)(int numMatchedChars,string lexeme), const char* err_string);
				
	 	//ADD ACCESSORS
	 	int checkMatch(const char* lexeme);
		Token makeTokenType(const char* lexeme);
